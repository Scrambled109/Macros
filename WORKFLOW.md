...
2. **Prepare cut files.** Use the Job Assistant DXF stage or
   `autocad/dxf-orchestrator/Master_Orchestrator.py`. Review every bevel and the
   orchestrator logs.
...
6. **Run AutoBOM.** Work on a recoverable model copy, then run the approved
   macro in `solidworks/auto-bom`. Review all updated properties and save
   results.
